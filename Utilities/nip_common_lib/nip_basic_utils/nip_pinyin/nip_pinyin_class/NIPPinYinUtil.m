...
#import "NIPPinYinUtil.h"
#import "nip_pinyin.h"

@implementation NIPPinYinUtil

+ (NSString *)pinYinStringSimpleOfWord:(NSString *)word {
    NIPHanyuPinyinOutputFormat *outputFormat=[[NIPHanyuPinyinOutputFormat alloc] init];
    [outputFormat setToneType:ToneTypeWithoutTone];
    [outputFormat setVCharType:VCharTypeWithV];
    [outputFormat setCaseType:CaseTypeLowercase];
    [outputFormat setBracketType:BracketTypeNone];
    return [self pinYinStringOfWord:word withHanyuPinyinOutputFormat:outputFormat];
}
...
+ (NSString *)pinYinStringComplexOfWord:(NSString *)word {
    ...
    [outputFormat setCaseType:CaseTypeLowercase];
    [outputFormat setBracketType:BracketTypeExist];
    return [self pinYinStringOfWord:word withHanyuPinyinOutputFormat:outputFormat];
}

+ (NSString *)pinYinStringOfWord:(NSString *)word withHanyuPinyinOutputFormat:(NIPHanyuPinyinOutputFormat *)outputFormat {
    NSMutableString *wordPinyinString = [NSMutableString string];
    NSString *result;
    for (int i = 0; i < word.length; i++) {
        unichar character = [word characterAtIndex:i];
        result = [[NIPChineseToPinyinResource getInstance] getHanyuPinyinRecordFromCharWithChar:character];
        [wordPinyinString appendString:[NIPinyinFormatter formatHanyuPinyinWithNSString:result withHanyuPinyinOutputFormat:outputFormat]];
    }
...
{
    NSMutableArray* wordPinyinArray = [NSMutableArray arrayWithCapacity:word.length];
    
    for (int i = 0; i < word.length; i++) {
        unichar character = [word characterAtIndex:i];
        NSArray* pinyinArray = [NIPinyinHelper getFormattedHanyuPinyinStringArrayWithChar:character withHanyuPinyinOutputFormat:outputFormat];
        [wordPinyinArray addObject:pinyinArray];
    }
    return wordPinyinArray;
}

+(BOOL)matchPinYin:(NSString*)pinyin withWord:(NSString*)word
{
    NSString* specialWhite = [NSString stringWithFormat:@"%C", 8198];
    pinyin = [pinyin stringByReplacingOccurrencesOfString:specialWhite withString:@""];//去掉中文输入的时候拼音之间的特殊空格
    ...
    NSArray* wordPinyinArray = [self pinYinArrayOfWord:word withHanyuPinyinOutputFormat:outputFormat];
    
    return [self matchPinYin:pinyin withWordPinYinArray:wordPinyinArray];
}

+(BOOL)matchPinYin:(NSString*)pinyin withWordPinYinArray:(NSArray*)wordPinyinArray
{
    //检查拼音的首字母是否匹配
    if (pinyin.length <= wordPinyinArray.count) {
        BOOL wordMatch = YES;
        for (int i = 0; i < pinyin.length; i++) {
            unichar word = [pinyin characterAtIndex:i];
            NSArray* wordArray = [wordPinyinArray objectAtIndex:i];
            BOOL firstMatch = NO;
            for (NSString* pinyinInArray in wordArray) {
                if ([pinyinInArray characterAtIndex:0] == word) {
                    firstMatch = YES;
                    break;
                }
            }
    ...
    //检查完整拼音是否匹配
    if ([NIPPinYinUtil matchwords:wordPinyinArray forStartIndex:0 forRemainString:pinyin]) {
        return YES;
    }
...
+(BOOL)matchwords:(NSArray*)wordsArray forStartIndex:(int)startIndex forRemainString:(NSString*)remainString
{
    if (startIndex >= wordsArray.count) {
        return NO;
    }
    ...
    BOOL wordMatch = NO;
    NSArray* oneWordArray = [wordsArray objectAtIndex:startIndex];
    for (NSString* wordPinyin in oneWordArray) {
        
        if (remainString.length > wordPinyin.length) {
            NSRange range = [remainString rangeOfString:wordPinyin];
            if (range.location == 0) {
                NSString* subRemainStr = [remainString substringFromIndex:range.location + range.length];
                wordMatch = [self matchwords:wordsArray forStartIndex:startIndex + 1 forRemainString:subRemainStr];
                if (wordMatch) {
                    break;
                }
            }
        ...
        {
            NSRange range = [wordPinyin rangeOfString:remainString];
            if (range.location == 0) {
                wordMatch = YES;
                break;
            }
...
//    NSData *gb2312_data = [word dataUsingEncoding:enc];
//    
