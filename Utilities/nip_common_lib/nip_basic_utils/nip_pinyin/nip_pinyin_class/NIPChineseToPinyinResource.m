...
#import "NIPChineseToPinyinResource.h"
#import "nip_macros.h"
...
- (id<NSCoding>)cachedObjectForKey:(NSString*)key
{
   ...
   // AMAssert4((!error), @"Error, s is %@, %s, %s, %d",error.description, __FILE__ ,__FUNCTION__, __LINE__);
    if (!error) {
        if (data) {
...
-(void)cacheObjec:(id<NSCoding>)obj forKey:(NSString *)key
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSError *error=nil;
            [data writeToFile:cachePath options:NSDataWritingFileProtectionComplete error:&error];
            if (error){
                PYLog(@"Error, s is %@, %s, %s, %d",error.description, __FILE__ ,__FUNCTION__, __LINE__);
            }
    });
...
+ (NIPChineseToPinyinResource *)getInstance {
    static NIPChineseToPinyinResource *sharedInstance=nil;
