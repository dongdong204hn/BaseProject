...
+ (NSString *)formatHanyuPinyinWithNSString:(NSString *)pinyinStr
    ...
    if (ToneTypeWithoutTone == [outputFormat toneType]) {
        ...
        if ([pinyinStr rangeOfString:@","].length) { //如果是多音字，没有音调时，相同的拼音应该去重
            NSArray *pinyins = [[pinyinStr stringByReplacingRegexPattern:@"[()]" withString:@""] componentsSeparatedByString:@","];
            NSMutableDictionary *pyDict = [NSMutableDictionary dictionary];
            for (NSString *py in pinyins) {
                [pyDict setValue:@"" forKey:py];
            }
            NSMutableString *pyStr = [NSMutableString stringWithString:@"("];
            for (NSString *key in pyDict.allKeys) {
                [pyStr appendString:[NSString stringWithFormat:@"%@,", key]];
            }
            [pyStr deleteCharactersInRange:NSMakeRange(pyStr.length - 1, 1)];
            [pyStr appendString:@")"];
            pinyinStr = pyStr;
        }
    }
        ...
        pinyinStr = [NIPinyinFormatter convertToneNumber2ToneMarkWithNSString:pinyinStr];
    }
    ...
    if (BracketTypeNone == [outputFormat bracketType]) {
        pinyinStr = [[pinyinStr stringByReplacingOccurrencesOfString:@"(" withString:@""] stringByReplacingOccurrencesOfString:@")" withString:@""];
    }
...
//        if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
//            int tuneNumber = [NIPinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length - 2]];
...
//                [resultBuffer appendString:[[lowerCasePinyinStr substringWithRange:NSMakeRange(indexOfUnmarkedVowel + 1, lowerCasePinyinStr.length-indexOfUnmarkedVowel - 1)] stringByReplacingOccurrencesOfString:@"v" withString:@"ü"]];
//                [resultBuffer replaceOccurrencesOfString:@"[1-5]" withString:@"" options:NSRegularExpressionSearch range:NSMakeRange(0, resultBuffer.length)];
...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
    ...
    if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
        ...
        NSString *allUnmarkedVowelStr = @"aeiouv";
        NSString *allMarkedVowelStr = @"āáăàaēéĕèeīíĭìiōóŏòoūúŭùuǖǘǚǜü";
        NSInteger tuneNumber;
        NSDictionary *unmarkedVowelDict;
        NSMutableString *resultBuffer = [[NSMutableString alloc] init];
        NSArray *pinyins = [lowerCasePinyinStr componentsSeparatedByString:@","];
        for (NSString *pinyin in pinyins) {
            if ([pinyin matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
                tuneNumber = [[pinyin stringMatchRegex:@"[1-5]"] integerValue];
                unmarkedVowelDict = [self unmarkedVowelAndIndexInPinyin:pinyin];
                unmarkedVowel = [unmarkedVowelDict.allKeys[0] characterAtIndex:0];
                indexOfUnmarkedVowel = [unmarkedVowelDict.allValues[0] integerValue];
                if ((defautlCharValue != unmarkedVowel) && (defautlIndexValue != indexOfUnmarkedVowel)) {
                    ...
                    unichar markedVowel = [allMarkedVowelStr characterAtIndex:vowelLocation];
                    [resultBuffer appendString:[[pinyin substringToIndex:indexOfUnmarkedVowel] stringByReplacingOccurrencesOfString:@"v" withString:@"ü"]];
                    [resultBuffer appendFormat:@"%C",markedVowel];
                    [resultBuffer appendString:[[pinyin substringWithRange:NSMakeRange(indexOfUnmarkedVowel + 1, pinyin.length-indexOfUnmarkedVowel - 1)] stringByReplacingOccurrencesOfString:@"v" withString:@"ü"]];
                    [resultBuffer replaceOccurrencesOfString:@"[1-5]" withString:@"" options:NSRegularExpressionSearch range:NSMakeRange(0, resultBuffer.length)];
                }
                else {
                    return lowerCasePinyinStr;
...
+ (NSDictionary *)unmarkedVowelAndIndexInPinyin:(NSString *)pinyin {
    unichar defautlCharValue = '$';
    NSInteger defautlIndexValue = -1;
    unichar unmarkedVowel = defautlCharValue;
    NSInteger indexOfUnmarkedVowel = defautlIndexValue;
    ...
    unichar charE = 'e';
    NSString *ouStr = @"ou";
    NSInteger indexOfA = [pinyin indexOf:charA];
    NSInteger indexOfE = [pinyin indexOf:charE];
    NSInteger ouIndex = [pinyin indexOfString:ouStr];
    if (-1 != indexOfA) {
        indexOfUnmarkedVowel = indexOfA;
        unmarkedVowel = charA;
    }
        ...
        for (NSInteger i = [pinyin length] - 1; i >= 0; i--) {
            if ([[NSString valueOfChar:[pinyin characterAtIndex:i]] matchesPatternRegexPattern:@"[aeiouv]"]) {
                indexOfUnmarkedVowel = i;
                unmarkedVowel = [pinyin characterAtIndex:i];
                break;
            }
        }
    }
    return @{[NSString stringWithFormat:@"%c", unmarkedVowel]: [NSNumber numberWithInteger:indexOfUnmarkedVowel]};
}
