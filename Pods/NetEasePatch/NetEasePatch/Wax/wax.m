...
void wax_setup()
{
    // fixBug:在wax中不做拦截，影响CrashAnalysis的上报结果
...
// ---------------
    ...
    if (luaL_loadbuffer(L, stdlib, stdlibSize, "loading wax stdlib") ||
    ...
    if ([[env objectForKey:@"WAX_TEST"] isEqual:@"YES"]) {
        ...
        snprintf(appLoadString, sizeof(appLoadString),
                 "local f = '%s' require(f:gsub('%%.[^.]*$', ''))",
                 initScript);  // Strip the extension off the file.
...
void wax_clear()
{
    ...
    for (NSDictionary *dict in replacedMethodArray) {
        ...
        if (sel_str && ![sel_str isKindOfClass:[NSNull class]] && sel_objc_str &&
            ![sel_objc_str isKindOfClass:[NSNull class]] && typeDesc &&
            ![typeDesc isKindOfClass:[NSNull class]]) {
    ...
    for (NSDictionary *dict in modifiedClassArray) {
        ...
        class_setVersion(class, (int)version);
    }
...
static void addGlobals(lua_State *L)
{
    ...
    lua_pushstring(
        L, [[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"] UTF8String]);
    ...
    lua_pushstring(L, [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask,
                                                            YES) objectAtIndex:0] UTF8String]);
    lua_setglobal(L, "NSDocumentDirectory");

    lua_pushstring(L, [[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask,
                                                            YES) objectAtIndex:0] UTF8String]);
