...
- (void)start
{
    // Apple makes has a mandatory 240 second minimum timeout WTF?
...
- (void)callRedirectCallback:(NSURLResponse *)redirectResponse
{
    ...
    if (hasCallback && wax_pcall(L, 1, 0)) {
        ...
        luaL_error(L, "Problem calling Lua function '%s' from wax_http.\n%s",
                   WAX_HTTP_REDIRECT_CALLBACK_FUNCTION_NAME, error_string);
    }
...
- (BOOL)callLuaAuthCallback:(NSURLAuthenticationChallenge *)challenge
{
    ...
    if (hasCallback && wax_pcall(L, 1, 0)) {
        const char *error_string = lua_tostring(L, -1);
        luaL_error(L, "Problem calling Lua function '%s' from wax_http.\n%s",
                   WAX_HTTP_AUTH_CALLBACK_FUNCTION_NAME, error_string);
    }
...
- (void)callLuaProgressCallback
{
    ...
    if (lua_isnil(L, -1)) {
        ...
        @try {
            percentComplete =
                _data.length /
                [[[_response allHeaderFields] objectForKey:@"Content-Length"] floatValue];
        } @catch (NSException *e) {
        ...
        if (wax_pcall(L, 2, 0)) {
            const char *error_string = lua_tostring(L, -1);
            luaL_error(L, "Problem calling Lua function '%s' from wax_http.\n%s",
                       WAX_HTTP_PROGRESS_CALLBACK_FUNCTION_NAME, error_string);
        }
    }
...
- (void)callLuaCallback
{
    ...
    if (_format == WAX_HTTP_UNKNOWN) {
        ...
        } else if ([contentType hasPrefix:@"image/"] || [contentType hasPrefix:@"audio/"] ||
    ...
    if (_error) {
        ...
        if (!string)
            string = [[NSString alloc] initWithData:_data encoding:NSISOLatin1StringEncoding];
        if (!string)
            string = [[NSString alloc] initWithData:_data encoding:NSNonLossyASCIIStringEncoding];
        if (!string)
            string = [[NSString alloc] initWithData:_data encoding:NSNonLossyASCIIStringEncoding];
    ...
    if (hasCallback && wax_pcall(L, 3, 0)) {
        const char *error_string = lua_tostring(L, -1);
        luaL_error(L, "Problem calling Lua function '%s' from wax_http.\n%s",
                   WAX_HTTP_CALLBACK_FUNCTION_NAME, error_string);
    }
