...
static const struct luaL_Reg metaFunctions[] = {{NULL, NULL}};

static const struct luaL_Reg functions[] = {{"parse", parse}, {"generate", generate}, {NULL, NULL}};
...
static char *appendNamespaceToName(const char *name, xmlNs *ns)
{
    ...
    if (ns && ns->prefix) {
        prefixElementNameSize =
            strlen((const char *)name) + 1;  // 1 is added because of the namespace colon :
            ...
            case XML_ELEMENT_NODE: {
                ...
                for (; attribute; attribute = attribute->next) {
                    xmlChar *attributeValue =
                        xmlNodeListGetString(node->doc, attribute->children, YES);
                    char *attributeName =
                        appendNamespaceToName((const char *)attribute->name, attribute->ns);
                ...
                if (lua_isnil(L, -1)) {  // not on stack yet, everything is cool
                    ...
                    lua_rawseti(L, -2, (int)(lua_objlen(L, -2) + 1));  // ad the new node
...
static int parse(lua_State *L)
{
    ...
    int xmlLength = (int)(lua_objlen(L, 1) + 1);
...
static void createXML(lua_State *L, xmlDocPtr doc, xmlNodePtr node, char *textLabel,
                      char *attrsLabel);

static void createElement(lua_State *L, xmlDocPtr doc, xmlNodePtr node, const char *name,
                          char *textLabel, char *attrsLabel)
{
