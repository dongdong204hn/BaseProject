...
enum { state_begin, state_array, state_hash };
...
static yajl_callbacks callbacks = {push_null, push_boolean, push_integer, push_double, NULL,
                                   push_string, push_start_map, push_map_key, push_end_map,
                                   push_start_array, push_end_array};
...
static int parse(lua_State *L)
{
    ...
    return parse_string(L, (const unsigned char *)input, (unsigned int)len);
}

void json_parseString(lua_State *L, const char *input)
{
    parse_string(L, (const unsigned char *)input, (unsigned int)strlen(input));
}
