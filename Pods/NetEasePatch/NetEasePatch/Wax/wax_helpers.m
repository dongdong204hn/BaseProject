...
void wax_printTable(lua_State *L, int t)
{
    ...
    // if t is negative, we need to normalize
...
int wax_fromObjc_new(lua_State *L, const char *typeDescription, NSInvocation *invocation, int index)
{
    ...
    switch (typeDescription[0]) {
        ...
        case WAX_TYPE_CHAR: {
            ...
            char c = *(char *)&buffer;
            if (c <= 1)
                lua_pushboolean(L, c);  // If it's 1 or 0, then treat it like a bool
            else
                lua_pushinteger(L, c);
            break;
        }
        ...
        case WAX_TYPE_ID: {
            ...
            if ([NSStringFromClass((Class)[instance class]) hasSuffix:@"Block__"]) {
                lua_pushlightuserdata(L, buffer);
            } else {
        ...
        case WAX_TYPE_SELECTOR: {
            SEL buffer;
            [invocation getArgument:&buffer atIndex:index];
            lua_pushstring(L, sel_getName(buffer));
        } break;

        case WAX_TYPE_CLASS: {
            void *buffer;
            [invocation getArgument:&buffer atIndex:index];
            Class instance = *&buffer;
            wax_instance_create(L, instance, YES);
            break;
        }

        default:
            luaL_error(L, "Unable to convert Obj-C type with type description '%s'",
                       typeDescription);
            break;
    }
...
#define WAX_TO_INTEGER(_type_)                                                                     \
    *outsize = sizeof(_type_);                                                                     \
    value = calloc(sizeof(_type_), 1);                                                             \
    *((_type_ *)value) = (_type_)lua_tointeger(L, stackIndex);
#define WAX_TO_NUMBER(_type_)                                                                      \
    *outsize = sizeof(_type_);                                                                     \
    value = calloc(sizeof(_type_), 1);                                                             \
    *((_type_ *)value) = (_type_)lua_tonumber(L, stackIndex);
#define WAX_TO_BOOL_OR_CHAR(_type_)                                                                \
    *outsize = sizeof(_type_);                                                                     \
    value = calloc(sizeof(_type_), 1);                                                             \
    *((_type_ *)value) = (_type_)(lua_isstring(L, stackIndex) ? lua_tostring(L, stackIndex)[0]     \
                                                              : lua_toboolean(L, stackIndex));
...
void *wax_copyToObjc(lua_State *L, const char *typeDescription, int stackIndex, int *outsize)
{
    ...
    // if no outsize address set, treat it as a junk var
    if (outsize == nil) outsize = alloca(sizeof(int));
    ...
    switch (typeDescription[0]) {
        ...
        case WAX_TYPE_SELECTOR:
            // If no slector is passed it, just use an empty string
        ...
        case WAX_TYPE_CLASS:
            *outsize = sizeof(Class);
            ...
            if (lua_isuserdata(L, stackIndex)) {
                wax_instance_userdata *instanceUserdata = (wax_instance_userdata *)luaL_checkudata(
                    L, stackIndex, WAX_INSTANCE_METATABLE_NAME);
            ...
            switch (typeDescription[1]) {
                    ...
                    switch (lua_type(L, stackIndex)) {
                        ...
                        case LUA_TUSERDATA: {
                            wax_instance_userdata *instanceUserdata =
                                (wax_instance_userdata *)luaL_checkudata(
                                    L, stackIndex, WAX_INSTANCE_METATABLE_NAME);
                        ...
                        default:
                            luaL_error(L, "Can't convert %s to wax_instance_userdata.",
                                       luaL_typename(L, stackIndex));
                            break;
                    }
                    break;
                }
                default:
                    if (lua_islightuserdata(L, stackIndex)) {
                        ...
                        free(value);
                        value = NULL;
                        luaL_error(L, "Converstion from %s to Objective-c not implemented.",
                                   typeDescription);
                    }
            }
        ...
        case WAX_TYPE_ID: {
            *outsize = sizeof(id);
            ...
            switch (lua_type(L, stackIndex)) {
                ...
                case LUA_TUSERDATA: {
                    wax_instance_userdata *instanceUserdata =
                        (wax_instance_userdata *)luaL_checkudata(L, stackIndex,
                                                                 WAX_INSTANCE_METATABLE_NAME);
                    instance = instanceUserdata->instance;
                    break;
                }
                case LUA_TLIGHTUSERDATA: {
                    instance = lua_touserdata(L, stackIndex);
                    /* instance = lua_touserdata(L, -1);
                     源代码中这样写，对多个LUA_TLIGHTUSERDATA类型的参数，会产生每次只取栈顶同一个参数的错误
                     */
                    break;
                }
                case LUA_TFUNCTION: {
        ...
        case WAX_TYPE_STRUCT: {
            if (lua_isuserdata(L, stackIndex)) {
                wax_struct_userdata *structUserdata = (wax_struct_userdata *)luaL_checkudata(
                    L, stackIndex, WAX_STRUCT_METATABLE_NAME);
        ...
        default:
            luaL_error(L, "Unable to get type for Obj-C method argument with type description '%s'",
                       typeDescription);
            break;
    }
...
// You can't tell if there are 0 or 1 arguments based on selector alone, so pass in an SEL[2] for
// possibleSelectors
void wax_selectorsForName(const char *methodName, SEL possibleSelectors[2])
{
    // Add 2. One for trailing : and one for \0
    int strlength = (int)(strlen(methodName) + 2);
...
BOOL wax_selectorForInstance(wax_instance_userdata *instanceUserdata, SEL *foundSelectors,
                             const char *methodName, BOOL forceInstanceCheck)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        // There may be only one acceptable selector (i.e. methods with multiple keyword args)
        if (!selector) continue;

        BOOL addSelector = NO;
        if (instanceUserdata->isClass && (forceInstanceCheck || wax_isInitMethod(methodName))) {
            if ([instanceUserdata->instance instanceMethodSignatureForSelector:selector])
                addSelector = YES;
        } else {
    ...
    while (type_description[index]) {
        switch (type_description[index]) {
            ...
            case WAX_PROTOCOL_TYPE_ONEWAY:
                ...
                break;
            default:
                [NSException raise:@"Wax Error"
                            format:@"Unknown type encoding %c", type_description[index]];
                break;
        }
...
int wax_simplifyTypeDescription(const char *in, char *out)
{
    ...
    while (in[in_index]) {
        switch (in[in_index]) {
            ...
            // get rid of internal stucture parts
            case WAX_TYPE_POINTER: {
                out[out_index++] = in[in_index++];
                for (; in[in_index] == '^'; in_index++)
                    ;  // Eat all the pointers
                ...
                switch (in[in_index]) {
                    ...
                    case WAX_TYPE_STRUCT: {
                        ...
                        for (; openCurlies > 1 || (in[in_index] != WAX_TYPE_UNION_END &&
                                                   in[in_index] != WAX_TYPE_STRUCT_END);
                             in_index++) {
                            if (in[in_index] == WAX_TYPE_UNION || in[in_index] == WAX_TYPE_STRUCT)
                                openCurlies++;
                            else if (in[in_index] == WAX_TYPE_UNION_END ||
                                     in[in_index] == WAX_TYPE_STRUCT_END)
                                openCurlies--;
                        }
                        break;
...
int wax_pcall(lua_State *L, int argumentCount, int returnCount)
{
    ...
    // Insert error function before arguments
