...
static const struct luaL_Reg functions[] = {
    {"copy", copy}, {"create", create}, {"pack", pack}, {"unpack", unpack}, {NULL, NULL}};
...
wax_struct_userdata *wax_struct_create(lua_State *L, const char *typeDescription, void *buffer)
{
    ...
    if (typeDescription[0] == '{') {  // We can get a name from the type desciption
        ...
        if (endLocation) {
            int size = (int)(endLocation - &typeDescription[1]);
...
void wax_struct_setValueAt(lua_State *L, wax_struct_userdata *structUserdata, int offset,
                           int stackIndex)
{
    char *simplifiedTypeDescription = alloca(strlen(structUserdata->typeDescription) + 1);
    wax_simplifyTypeDescription(structUserdata->typeDescription, simplifiedTypeDescription);
...
int wax_struct_getOffsetForName(lua_State *L, wax_struct_userdata *structUserdata, const char *name)
{
    ...
    if (lua_isnil(L, -1)) {
        luaL_error(L, "No mapping for varible named '%s' for struct '%s'", name,
                   structUserdata->name);
    }
...
static int __index(lua_State *L)
{
    wax_struct_userdata *structUserdata =
        (wax_struct_userdata *)luaL_checkudata(L, 1, WAX_STRUCT_METATABLE_NAME);
...
static int __tostring(lua_State *L)
{
    ...
    if (lua_isnil(L, -1)) {
        ...
        while (lua_next(L, -2)) {
            ...
            luaL_addstring(&b, "\n");
            // pops the value and the struct offset, keeps the key for the next iteration
            lua_pop(L, 2);
        }
...
static int copy(lua_State *L)
{
    wax_struct_userdata *structUserdata =
        (wax_struct_userdata *)luaL_checkudata(L, 1, WAX_STRUCT_METATABLE_NAME);
    wax_struct_userdata *newStructUserdata =
        wax_struct_create(L, structUserdata->typeDescription, structUserdata->data);

    unsigned long size = strlen(structUserdata->name);
...
static int pack(lua_State *L)
{
    ...
    if (strlen(simplifiedTypeDescription) != lua_gettop(L) - 1) {
        luaL_error(L, "Couldn't pack struct. Received %d arguments for struct with type "
                      "description '%s' (should have received %d)",
                   lua_gettop(L) - 1, typeDescription, strlen(simplifiedTypeDescription));
    }
