...
static void TCPServerAcceptCallBack(CFSocketRef socket, CFSocketCallBackType type,
...
- (NSError *)startOnPort:(NSUInteger)port
{
    ...
    _ipv4socket = CFSocketCreate(kCFAllocatorDefault, PF_INET, SOCK_STREAM, IPPROTO_TCP,
                                 ...
                                 (CFSocketCallBack)&TCPServerAcceptCallBack, &socketCtxt);

    if (_ipv4socket == NULL) {
        error = [[NSError alloc] initWithDomain:@"Wax Error"
    ...
    // set up the IPv4 endpoint; use port 0, so the kernel will choose an arbitrary port for us,
    // which will be advertised using Bonjour
...
- (BOOL)send:(NSString *)output
{
    ...
    NSInteger length = [_outStream write:(uint8_t *)[output UTF8String]
                               maxLength:[output lengthOfBytesUsingEncoding:NSUTF8StringEncoding]];
    return length > 0;
}
...
- (BOOL)enableBonjourOnPort:(NSUInteger)port
{
    ...
    _netService =
        [[NSNetService alloc] initWithDomain:domain type:protocol name:name port:(int)port];
                          ...
                          outputStream:(NSOutputStream *)outStream
{
    if (_inStream || _outStream) {
        [NSException
             raise:@"Debug Server Error"
            format:@"Woah, a new connection came in. I have no idea what to do in this situation."];
        return;
    }
...
{
    return [NSString stringWithFormat:@"<%@ = 0x%08lX | port %ld | netService = %@>", [self class],
                                      (long)self, (long)[_netService port], _netService];
}
                                    ...
                                    CFDataRef address, const void *data, void *info)
{
    ...
    if (kCFSocketAcceptCallBack == type) {
        ...
        CFStreamCreatePairWithSocket(kCFAllocatorDefault, nativeSocketHandle, &readStream,
                                     &writeStream);
        if (readStream && writeStream) {
            ...
            // on any failure, need to destroy the CFSocketNativeHandle since we are not going to
            // use it any more
